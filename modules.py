...
class MultiKernelDepthWiseConvolution(nn.Module):
    def __init__(self, channels, expansionFactor, nChunks=1, stride=1, bias=True):
        ...
        self.layers.append(nn.Sequential(DepthWiseConvolution((channels*expansionFactor)//nChunks, 1, stride, 1), SqueezeAndExcitation(self.outChannels,self.outChannels//4), BatchNormalization2D(self.outChannels), MemoryEfficientSwish(), nn.Dropout2d(0.2)))#,  BatchNormalization2D((channels*expansionFactor)//nChunks),MemoryEfficientSwish()))
        self.layers.append(nn.Sequential(DepthWiseConvolution((channels*expansionFactor)//nChunks, 3, stride, 1), SqueezeAndExcitation(self.outChannels,self.outChannels//4), BatchNormalization2D(self.outChannels), MemoryEfficientSwish(), nn.Dropout2d(0.2)))#,  BatchNormalization2D((channels*expansionFactor)//nChunks), MemoryEfficientSwish()))
...
class Net(nn.Module):
    def __init__(self, nc, dp=0.2):
        ...
        self.head = nn.Conv2d(in_channels=3,out_channels=64,kernel_size=3, stride=2)
        self.swish = MemoryEfficientSwish()
        self.bn = BatchNormalization2D(32)
        self.channels = [32, 128]
        ...
        MemoryEfficientSwish(),
        MultiKernelDepthWiseConvolution(n,1,4,1),
        BatchNormalization2D(n),
        MemoryEfficientSwish(),
        MultiKernelDepthWiseConvolution(n,1,4,2),
        BatchNormalization2D(n),
        MemoryEfficientSwish(),
        PointWiseConvolution(n,n*4,2,1,True),
