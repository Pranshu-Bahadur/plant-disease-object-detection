...
class Net(nn.Module):
    def __init__(self, nc, dp=0.2):
        ...
        self.stages = nn.ModuleList([
            nn.Sequential(MBConv(stage[0], stage[0], 3, 1, 0, 6) if s is not stage[2] - 1 else MBConv(stage[0], stage[1], 3, 2, 0, 6) for s in range(stage[2])) for stage in self.config])
