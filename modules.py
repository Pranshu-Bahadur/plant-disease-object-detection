    ...
    def __init__(self, nc, dp=0.2):
        ...
        for i in range(len(self.config)):
            for j in range(self.config[i][2]):
                self.stages[i].add_module(str(j+1), MBConv(self.config[i][0], self.config[i][0], 3, 1, 0, 6) if j is not self.config[i][2] - 1 else MBConv(self.config[i][0], self.config[i][0], 3, 2, 0, 6))
