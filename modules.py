...
class MultiKernelDepthWiseConvolution(nn.Module):
    def __init__(self, channels, expansionFactor, nChunks=1, stride=1, bias=True):
        ...
        self.layers.append(nn.Sequential(DepthWiseConvolution((channels*expansionFactor)//nChunks, 5, stride, 1), BatchNormalization2D(self.outChannels), MemoryEfficientSwish()))#,  BatchNormalization2D((channels*expansionFactor)//nChunks), MemoryEfficientSwish()))
...
class Net(nn.Module):
    def __init__(self, nc, dp=0.2):
        ...
        self.final_conv = nn.Sequential(nn.Conv2d(self.config[-1][1], self.config[-1][1]*4, 1, 1))#, BatchNormalization2D(self.config[-1][1]), MemoryEfficientSwish())
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Linear(self.config[-1][1]*4, nc)
    def forward(self, x):
        x = self.head(x)
        x = self.bn(x)
        x = self.swish(x)
