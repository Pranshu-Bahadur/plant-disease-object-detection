...
class ImageClassifier(object):
    ...
    def _create_optimizer(self, name, model_params, lr):
        optim_dict = {"SGD":torch.optim.SGD(model_params.parameters(), lr,weight_decay=1e-5, momentum=0.9, nesterov=True),
                      ...
                      "SGDAGC": AGC(model_params.parameters(), torch.optim.SGD(model_params.parameters(), lr,weight_decay=1e-5, momentum=0.9, nesterov=True), model=model_params, ignore_agc=['head'], clipping=0.04)#SGD_AGC(model_params.parameters(), lr=lr, clipping=0.01, weight_decay=2e-05, nesterov=True, momentum=0.9) #,###
        }
    ...
    def _train_or_eval(self, loader, train):
        ...
        if train: #and (self.curr_epoch+1)%5==0:
            ...
            self.optimizer.param_groups[0]['clipping'] = self.optimizer.param_groups[0]['clipping']*2  if self.bs>128 or self.curr_epoch==self.final_epoch-2 else 0.32
