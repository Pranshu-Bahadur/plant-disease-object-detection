    ...
    def _train_or_eval(self, loader, train):
        ...
        for idx, data in enumerate(loader):
            ...
            if train:
                ...
                list(map(lambda img: torchvision.transforms.ToTensor()(self.RA_Helper(torchvision.transforms.Resize(self.resolution - 32*self.counter)(torchvision.transforms.ToPILImage()(img)), self.counter, 0, idx)), x)) if self.counter > 0 else list(map(lambda img: torchvision.transforms.ToTensor()(self.RA_Helper(torchvision.transforms.ToPILImage()(img), self.counter, 0, idx)), x))
                #x = torch.stack(x)
                #x = x[shuffle_seed]
