...
class ImageClassifier(object):
    ...
    def _create_optimizer(self, name, model_params, lr):
        optim_dict = {"SGD":torch.optim.SGD(model_params.parameters(), lr,weight_decay=1e-5, momentum=0.9, nesterov=True),
                      ...
                      "SGDAGC": SGD_AGC(model_params.parameters(), lr=lr, clipping=0.04) #, weight_decay=2e-05, nesterov=True, momentum=0.9,##AGC(model_params.parameters(), torch.optim.SGD(model_params.parameters(), lr), model=model_params, ignore_agc=['head'], clipping=0.04)#
        }
    ...
    def _train_or_eval(self, loader, train):
        ...
        for idx, data in enumerate(loader):
            ...
            if train:
                x_ = []
                for i in range(3):
                    x_.torch.stack([torchvision.transforms.ToTensor()(self.RA_Helper(torchvision.transforms.Resize(self.resolution - 32*self.counter,interpolation=PIL.Image.ANTIALIAS)(torchvision.transforms.ToPILImage()(img)), self.counter, i, idx)) for img in x[y==i]])
                x = x_[torch.randperm(x_size(0))]
                print(x.size())
    ...
    def RA_Helper(self, x, i, j, idx):
        if idx==0:
            torchvision.utils.save_image(torchvision.transforms.ToTensor()(x), "/home/fraulty/ws/RP_TBX11K/content/Before_RA_{}_{}.png".format(self.resolution - 32*self.counter, j))
        for _ in range(3 - i):
            x = RandAugment()(x)
        if idx==0:
            torchvision.utils.save_image(torchvision.transforms.ToTensor()(x), "/home/fraulty/ws/RP_TBX11K/After_RA_{}_{}.png".format(self.resolution - 32*self.counter, j))
        return x