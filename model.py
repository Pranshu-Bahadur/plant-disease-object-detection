...
class ImageClassifier(object):
    ...
    def _create_optimizer(self, name, model_params, lr):
        optim_dict = {"SGD":torch.optim.SGD(model_params.parameters(), lr,weight_decay=1e-5, momentum=0.9, nesterov=True),
                      ...
                      "SGDAGC": SGD_AGC(model_params.parameters(), lr=lr, clipping=0.01) #, weight_decay=2e-05, nesterov=True, momentum=0.9,##AGC(model_params.parameters(), torch.optim.SGD(model_params.parameters(), lr), model=model_params, ignore_agc=['head'], clipping=0.04)#
        }
