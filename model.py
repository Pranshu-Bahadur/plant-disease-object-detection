    ...
    def _train_or_eval(self, loader, train):
        ...
        for idx, data in enumerate(loader):
            ...
            if train:
                ...
                x = list(map(lambda img: torchvision.transforms.functional.pil_to_tensor(self.RA_Helper(torchvision.transforms.functional.to_pil_image(img), self.counter, 0, idx)), x))
                x = torch.stack(x)
                #x = x[shuffle_seed]
