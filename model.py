        ...
        for idx, data in enumerate(loader):
            ...
            if train:
                ...
                for i in range(3):
                    if x[y==i].size(0) > 0:
                        for _ in range(4):
                            x_.append(torch.stack([torchvision.transforms.ToTensor()(self.RA_Helper(torchvision.transforms.Resize(self.resolution - 64*self.counter,interpolation=PIL.Image.ANTIALIAS)(torchvision.transforms.ToPILImage()(img)), self.counter, i, idx)) for img in x[y==i]]))
                            y_.append(y[y==i])
